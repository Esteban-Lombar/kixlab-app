...
export default function Home() {
  ...
  // ✅ Carga productos y muestra los últimos primero
  useEffect(() => {
    let alive = true;
    setLoading(true);
    setError("");
    ...
    fetch("/data/products.json", { cache: "no-store" })
      ...
      .catch((e) => {
        ...
        if (alive)
          setError(
            "No pudimos cargar el catálogo. Revisa /public/data/products.json"
          );
      })
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      const matchCat =
        category === "Todos" || (p.category || []).includes(category);
              ...
              >
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="font-medium text-[13px] leading-snug truncate">
                      {p.name}
                    </h3>
                  </div>
